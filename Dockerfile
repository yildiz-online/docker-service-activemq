...
ENV ACTIVEMQ apache-activemq-5.15.12

RUN mkdir /usr/src

RUN curl -fsSL -o activemq.tar.gz \
		"https://archive.apache.org/dist/activemq/5.15.12/apache-activemq-5.15.12-bin.tar.gz" \
    && tar xf activemq.tar.gz -C /usr/src/ \
    && mv /usr/src/apache-activemq-5.15.12 /opt/activemq \
